    ...
    describe('- ' + specArea + ':', function() {
      ...
      specs.tests.forEach(function(test) {
        ...
        it_(test.name + ' - ' + test.desc, function() {
          if (test.timeInterval.lambda && test.timeInterval.lambda.__tag__ === 'code')
            test.timeInterval.lambda = eval('(function() { return ' + test.timeInterval.lambda.js + '; })');
          var output = Mustache.render(test.template, test.timeInterval, test.partials);
